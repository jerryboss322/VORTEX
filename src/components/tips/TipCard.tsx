...
export function TipCard({ tip }: { tip: Tip }) {
  ...
  return (
    <article className="overflow-hidden rounded-[18px] border border-[var(--th-border)] bg-[var(--th-surface)]">
      {/* accent strip */}
      <div className={`h-[3px] w-full ${accent}`} aria-hidden />

      {/* optional slip thumb — kept subtle, not dominant; ticket hero is the data */}
      ...
      <div className="grid grid-cols-[1.2fr_0.9fr] gap-4 px-5 pt-3">
        ...
        <div className="text-right">
          ...
          {payoutExample(odds) && tip.confidence == null && (
            <div className="text-[12px] text-[var(--th-sub)]">{formatDate(tip.createdAt)}</div>
          )}
        </div>
      </div>
      ...
      {/* footer — booking code chip + ghost Copy pill */}
