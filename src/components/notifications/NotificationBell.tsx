  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--th-gold)] px-1 text-[10px] font-[600] text-[#0E1013]">{unread > 99 ? "99+" : unread}</span>
        )}
      </button>

      {open && (
        <div className="absolute right-0 mt-3 w-[380px] max-w-[92vw] overflow-hidden rounded-[14px] border border-[var(--th-border)] bg-[var(--th-surface)] z-50">
            ...
            {filtered.length === 0 ? (
              ...
              <ul className="divide-y divide-[var(--th-border)]/60">
                {filtered.map((n) => {
                  ...
                  return (
                    <li key={n.id} className={`group relative flex gap-3 px-4 py-3 hover:bg-[var(--th-chip)]/50 transition-colors ${!n.read ? "bg-[var(--th-chip)]/30" : ""}`}>
                      {!n.read && <span className={`absolute left-0 top-0 h-full w-0.5 ${m.dot}`} />}
                      <div className={`flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[9px] border border-[var(--th-border)] text-[13px] ${!n.read ? "bg-[var(--th-chip)] text-[var(--th-text)]" : "bg-transparent text-[var(--th-sub)]"}`}>{m.icon}</div>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-2">
                          ...
                          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-[400] ${!n.read ? "bg-[var(--th-gold)] text-[#3A2E14]" : "bg-[var(--th-chip)] text-[var(--th-sub)]"}`}>{m.label}</span>
                        </div>
                        {n.link && (
